...
export default function SignIn() {
  ...
  const SignInUser = async ({ email, password }) => {
    ...
    try {
      ...
      if (error) {
        toast.error(error.message === 'Email not confirmed' ? t('check-email-verify') : error.message, {
  ...
  return (
    <div className="bg-[#2c2c2c] my-3 flex justify-center items-center rounded-[15px] w-full h-[30rem]">
      <div className="h-full w-full mt-20 lg:mt-16">
          <div className="pt-4 text-center">
            <p className="text-3xl text-[#ffd601] font-semibold font-title">{t('sign-in')}</p>
          </div>
          <div
          ...
          >
            ...
            {({ values, handleChange, errors, touched, isValid, setFieldTouched, handleSubmit }) => (
              <div className="flex flex-col text-[#ffff]">
                ...
                <input
                  type="password"
                  className={`border placeholder:text-xs text-xs placeholder:text-[#bebebe] font-semibold ${i18n.language === 'ar' ? 'pr-3' : 'pl-3'} w-full lg:w-96 mt-4 outline-none rounded-[5px] h-14 border-[#d3d3d3] bg-[#2c2c2c]`}
                ...
                <div className="pb-6 flex items-center justify-center">
                  <button onClick={isValid ? handleSubmit : null} type="submit" className={`bg-[#ffd601] hover:bg-[#d1b736] ${i18n.language === 'ar' ? 'ml-3' : 'mr-3'} mt-4 w-full lg:w-96 outline-none rounded-[5px] h-14 text-black font-semibold text-base`}>
