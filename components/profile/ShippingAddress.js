...
export default function ShippingAddress() {
  ...
  async function InsertShippingAddress(e) {
    e.preventDefault()
    try {
  ...
  const UpdateShippingAddress = async () => {
    if (shippingAddress === newShippingAddress) return toast('No change in address', {
      position: "top-right",
      autoClose: 5000,
    ...
    try {
      const mutation = `mutation MyMutation ($id: uuid!, $newShippingAddress: jsonb!) {
            update_Profiles_by_pk(pk_columns: {user_id: $id} _set: { shippingAddress: $newShippingAddress}) {
                shippingAddress
            }
        }`;
      const variables = {
  ...
  return (
    <div>
      ...
      <form onSubmit={InsertShippingAddress}>
