...
<div class="flex min-h-screen bg-gray-100">
    ...
    ${initiated_orders.data.promo_code_used?`AED${amount}<br/>(Promocode used)`:`AED${amount}`}
      ...
      var document = {
        ...
        .then(async (pdfReceipt) => {
            console.log(pdfReceipt);
            const data1 = {
                from: 'travo.socialmedia@gmail.com',
                personalizations: [
                    {
                        to: [`${profile.data[0].email}`, 'anandhu@rough-paper.com'],
                        subject: 'Order Confirmation'
                    },
                ],
                content: [{ type: "text/html", value: header + body1 + footer + body2 },],
                // content: [{"type": "text/html", "value": document.toString() }],
                attachments: [
                    {
                        content: pdfReceipt.toString('base64'),
                        filename:`KUKU${String(6 + 1).padStart(7, '0')}.pdf`,
        ...
        return res.json({ document, options})
//         let initiated_orders = await supabase
//             .from('initiated_orders')
//             .select('*')
//             .eq("verification_secret", "pi_3KajULLSsCUq84XE0HHU8kmn")
